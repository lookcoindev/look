LookCoin-qt: Qt4 GUI for LookCoin
===============================
...
Alternatively, install Qt Creator and open the `LookCoin-qt.pro` file.

An executable named `LookCoin-qt` will be built.
...
A warning for people using the *static binary* version of LookCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LookCoin is linked against libdb5.3.
...
installed causes LookCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
