...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LookCoinTEST2\r");
            Send(hSocket, "WHO #LookCoinTEST2\r");
        } else {
            // randomly join #LookCoin00-#LookCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LookCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LookCoin%02d\r", channel_number).c_str());
        }
